...
EOS
    ...
    def execute_once
      $stdout.puts "Starting replication run"
      starting = Time.new
      session.refresh
      ...
      terminated = TaskSweeper.timeout(timeout) do |sweeper|
        ...
        run.run
        $stdout.puts "Replication finished #{Time.new - starting}s"
      end.terminated?
    ...
    def execute
      init_waiter
      $stdout.puts "Preparing replication"
      prepare_replication
      replication_preparation_finished
      $stdout.puts "Starting replication loop"
      until termination_requested do
        ...
        # $stdout.puts "Stopping replication for next #{session.configuration.options[:replication_interval]} seconds"
        pause_replication
