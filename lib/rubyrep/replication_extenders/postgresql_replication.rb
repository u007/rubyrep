  ...
  module ReplicationExtenders
    ...
    module PostgreSQLReplication
      ...
      def key_clause(trigger_var, params)
        # $stdout.puts "key_clause: #{trigger_var}: #{params.inspect}"
        if params[:keys].empty?
          raise "No unique / primary key columns specified for table #{params.inspect}"
        end
        params[:keys].
      ...
      def sequence_values(rep_prefix, table_name)
        ...
        sequence_names.each do |sequence_name|
          row = select_one("select last_value, NEXTVAL('#{sequence_name}') as next_value from \"#{sequence_name}\"")
          result[sequence_name] = {
            # :increment => row['increment_by'].to_i,
            :increment => row['next_value'].to_i - row['last_value'].to_i,
            :value => row['last_value'].to_i
          }
        end
        $stdout.puts "sequence_values: #{table_name}: #{result.inspect}"
        result
      end
      ...
      def update_sequences(
        ...
        left_sequence_values.each do |sequence_name, left_current_value|
          row = select_one("select last_value, NEXTVAL('#{sequence_name}') as next_value from \"#{sequence_name}\"")
          next_value = row['next_value'].to_i
          ...
          unless current_increment == increment and current_value % increment == offset
            ...
            $stdout.puts "alter sequence #{sequence_name} increment by #{increment} restart with #{new_start}"
            execute(<<-end_sql)
      ...
      def clear_sequence_setup(rep_prefix, table_name)
        ...
        end_sql
        sequence_names.each do |sequence_name|
          $stdout.puts "reset excluded: alter sequence #{sequence_name} increment by 1"
          execute(<<-end_sql)
